...
export class LeafletMap {
  ...
  initMarkers() {
    ...
    initialMarkers.forEach((data, index) => {
      ...
      marker
        .addTo(this.map)
        .bindPopup(
          `<div class="infoBox"><div class="marker-detail"><img src="${data.map_image_url}" alt="Image"><div class="detail-part"><h6>${data.name}</h6><div class="rating ${data.rating}"><ngb-rating [rate]="${data.rate}"></ngb-rating></div><span>${data.price}</span><a href="javascript:void(0)">Details</a></div></div></div> `,
        );
  ...
  generateMarker(data: { position: L.LatLngExpression; draggable: boolean }, index: number) {
    return L.marker(data.position, { draggable: data.draggable })
      ...
      .on('dragend', (event: L.DragEndEvent) => this.markerDragEnd(event, index));
  }
  ...
  markerDragEnd(_$event: L.DragEndEvent, _index: number) {}
}
